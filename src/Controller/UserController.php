...
class UserController extends AbstractController
{
    ...
    public function loadUserContent(UserRepository $userRepository, int $iduser): Response
    {   
        ...
        // Get user data for QR code generation
        $userData = $user->getUserDataForQrCode();
    
        // Create a QR code instance
        $qrCode = new QrCode($userData);
    
        // Use the PngWriter service to generate the QR code image
        $pngWriter = $this->get(PngWriter::class);
        $pngResult = $pngWriter->write($qrCode);
    
        // Encode the QR code image as base64
        $qrCodeImage = base64_encode($pngResult->getString());
    
        // Render the template with the user and QR code image
